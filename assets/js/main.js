...
async function fetchLanyardData() {
  ...
  try {
    const response = await fetch(
      "https://api.lanyard.rest/v1/users/1326116558166032424",
    );
    ...
    if (data) {
      ...
      if (spotifyActivity) {
        discordActivity.innerHTML = `
          <a href="https://open.spotify.com/track/${spotifyActivity.sync_id}" target="_blank">
            <img src="https://cdn-icons-png.flaticon.com/512/174/174872.png" alt="Spotify Icon">
            <span>Listening to Spotify</span>
            <span><strong>${spotifyActivity.details}</strong> - ${spotifyActivity.state}</span>
          </a>
        `;
      } else {
        discordActivity.innerHTML = `<p>No activity detected.</p>`;
      }
    }
  } catch (error) {
    console.error("Error fetching Discord data:", error);
    discordActivity.innerHTML = "<p>Failed to load Discord data.</p>";
  }
}
